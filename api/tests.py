...
class ValidateAndReturnWCNFTestCase(TestCase):
    ...
    def test_validate_and_return_wcnf_with_valid_clause_list(self):
        ...
        clauses = [{'variables': clause_data[0], 'weight': clause_data[1]} for clause_data in clauses_data]
        request_data = json.dumps(clauses)

        result_clauses = validate_and_return_wcnf(request_data)

        self.assertListEqual(result_clauses, clauses)
    ...
    def test_validate_and_return_wcnf_clause_with_no_weight(self):
        ...
        clauses = [{'variables': clause_data[0], 'weight': None if len(clause_data)==1 else clause_data[1]} for clause_data in clauses_data]
        request_data = json.dumps(clauses)
